...
const envSchema = z.object({
  ...
  // Required in production; optional in dev/test so the backend (and the
  // migrate.ts spawned by tests/global-setup.ts) can boot without Eskiz
  // creds when SMS isn't being exercised. Mirrors the RESEND_API_KEY pattern.
  SMS_API_EMAIL: isDev
    ? z.string().default('')
    : z.string().min(1, 'SMS_API_EMAIL is required'),
  SMS_API_PASSWORD: isDev
    ? z.string().default('')
    : z.string().min(1, 'SMS_API_PASSWORD is required'),
